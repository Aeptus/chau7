...
struct RemoteFrame {
    ...
    func headerBytes(payloadLen: UInt32) -> Data {
    ...
    static func decode(from data: Data) throws -> RemoteFrame {
        guard data.count >= headerSize else {
            throw RemoteFrameError.insufficientData
        }
        let payloadLen = Int(data.readUInt32LE(at: 16))
        let expected = headerSize + payloadLen
        guard data.count >= expected else {
            throw RemoteFrameError.invalidLength
        }
        let payload = data.subdata(in: headerSize..<expected)
        return RemoteFrame(
            version: data[0],
            type: data[1],
            flags: data[2],
            reserved: data[3],
            tabID: data.readUInt32LE(at: 4),
            seq: data.readUInt64LE(at: 8),
            payload: payload
        )
    }
}

enum RemoteFrameError: Error {
