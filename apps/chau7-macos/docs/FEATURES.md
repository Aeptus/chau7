...
- Optional iCloud sync across devices — freshness-guarded: only blobs strictly newer than this Mac's last synced state apply, newer-format exports are refused, fields absent from a blob keep their local value instead of resetting to defaults, Sync Now performs an immediate push, and sync/restore/import/export actions report visible status in the pane.
