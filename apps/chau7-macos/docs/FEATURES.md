...
- **Billable-endpoint token accounting** — token and cost estimation runs only for endpoints that return model output. Catalog listings (`/v1/models`), token counters, and other non-completion routes record no usage, so frequently polled endpoints cannot inflate token totals or spend. Classification is an allowlist: an unrecognized route records nothing rather than an invented estimate.
