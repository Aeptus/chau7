...
- **Proactive idle-tab scrollback flush (default on)** — a settled, idle non-selected tab demotes to `.hidden` and flushes its scrollback ring to disk (keeping a ~200-line resident floor for instant context on return; the rest reloads lazily), instead of holding the full configured ring resident. A tab running/waiting on an AI session stays at full fidelity, and tabs still starting up are left alone. In many-tab sessions this is the bulk of the footprint — measured ~250 MB lighter. Reversible via Settings (Suspend Background Rendering).
