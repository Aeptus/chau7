...
- **Codex feedback prompt detection** — structured Codex `request_user_input` records provide authoritative waiting-input evidence, while completed turns that conservatively end in an explicit choice or confirmation request are reclassified as heuristic waiting-input events instead of false completions.
...
- **Completed-turn tab styling** — a finished Claude turn styles its tab with the green success preset (non-persistent, auto-clearing, suppressed once the tab is viewed) rather than the orange "waiting" style, so a completed agent no longer glows as if it were still blocked on you. The turn-completion fallback resolves to `task_finished` (not a synthetic `waiting_input`), and the PTY wait/approval heuristic matches only the trailing output region so a reply that merely contains a trigger word ("Proceed?", "[y/N]") can't flip a finished turn back to waiting-for-input.
...
- Repo-grouped iPhone tab menu — the tab dropdown sections tabs under their repo name, preserving the Mac's tab order, with repo-less tabs under "Other".
