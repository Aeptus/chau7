...
- Responsive settings shell keeps the sidebar, search results, section copy, and form controls usable while resizing, with adaptive rows, wrapping text for longer localized labels and accessibility descriptions, a compact native titlebar profile selector that defaults to "Default Settings" and owns profile load/save actions, and shared golden-ratio SwiftUI style tokens for settings spacing, radius, cards, hints, and badges.
