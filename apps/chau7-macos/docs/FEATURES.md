...
| **Metal OSC8 and local-echo parity** | Metal sync overlays predicted local-echo cells before GPU conversion, immediately invalidates the active Metal surface on overlay changes, and renders OSC8 link underlines when no explicit SGR underline is present |
