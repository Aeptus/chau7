...
- **Tab-scoped runtime session uniqueness** — each tab has at most one active runtime session; stopped sessions preserve replacement mappings, provider-session indexes stay consistent, and dashboard refresh handles legacy duplicate snapshots without crashing.
- **Aethyme pull-request activity delivery** — a background adapter polls Aethyme only for repositories represented by live tabs, advances due PR watches, and claims provider-neutral outbox items with generation fencing. Delivery requires the exact subscribed tab, AI session, repository, idle/done state, ready prompt, and empty staged MCP input; the command-filtering buffer retains partial MCP fragments until submit or cancel, busy targets retry, and identity drift fails closed. Aethyme retains observation, deduplication, authorization policy, and durable delivery ownership so Chau7 remains one replaceable delivery client.
- **Live AI session binding validation** — provider hook events must agree with the AI provider currently displayed in their stamped terminal tab. Exact-session, stamped-tab, and directory attribution all reject conflicting live providers, and runtime conflict invalidation cannot fall through into re-adoption of the same tab; stale hook bursts therefore remain quarantined while shell returns and same-provider restarts stay available.
...
- **Restore-sanitized AI fallback state** — the restore boundary validates provider/session metadata once, then autosave trusts that in-memory fallback and rebuilds only canonical commands without rescanning transcript directories. Obsolete or foreign identities are removed before entering the fallback cache.
...
- **Terminal background-work profiler** — backend output reads, grid snapshots, cursor-mode checks, persistence captures, and terminal-state processing are aggregated by render phase, visibility, and caller. One identity-free structured sample per minute records processed bytes plus total, main-thread, and maximum duration in the separate bounded `Chau7-performance.jsonl`, making hidden-tab work measurable without consuming operational-log history.
...
- **Stale session source quarantine** — a mismatched AI hook stamp receives one exact-session reroute attempt; unresolved repeats are rejected without repeated routing or per-event warnings and produce only a bounded once-per-minute health summary until ownership recovers.
...
- **Single-owner transcript extraction** — initial background extraction reserves the run before publishing completion. Periodic and explicit repairs wait for that ownership to end; a missing transcript remains retryable without being marked complete.
...
- **Foreground-only hardware input** — terminal key routing requires Chau7 to be active and the targeted key window to be on the active Space, in addition to terminal first-responder ownership, so retained AppKit focus state cannot leak typing into a background PTY.
...
- Terminal copy, paste, password AutoFill, and text input remain explicit responder actions without advertising a macOS Services endpoint, avoiding synchronous ShareKit extension discovery during focus and menu validation.
...
- Hard lossless memory ceilings: warm non-TUI scrollback flushes persist-before-shrink above 64 MB per tab, derived search/line caches regenerate above 16 MB per pane, and fully invisible windows release their renderer allocation while retaining the latest triple-buffered frame for instant reveal.
...
- Generation-based dirty-row rendering: Rust retains Alacritty damage per viewport row for independent split-view consumers, while Metal and CPU renderers ingest packed changed rows and recover authoritatively on cold start, resize, or theme changes without copying an unchanged full grid.
- Dedicated performance telemetry: terminal and renderer aggregates publish once per minute to an 8 MB JSONL stream with one complete archive and no tab/session identities; only invariant violations remain in the operational log.
...
| **Generation-based Rust snapshots** | Packed dirty rows advance each display independently; rotating buffers catch up missed row generations without stale content |
| **Persistent MCP connections** | Initialized local Unix-socket sessions stay open until a peer closes them; only writes retain a timeout, while successful connection/handshake churn is trace-only |
...
| **Off-main terminal frame preparation** | Rust snapshot acquisition, cursor reads, row tinting, and cell conversion run on a serial user-interactive queue; AppKit presents only a generation-owned completed frame while PTY state remains lossless |
| **Ordered off-main session durability** | AppKit state capture stays brief; restore-index encoding, bundle publication, backups, and clearing run on one serial utility queue, while termination drains it before shells close |
| **Lossless hang circuit breaker** | Confirmed event-loop stalls pause only terminal frame preparation/presentation; coalesced paint resumes from the newest authoritative PTY state after heartbeat recovery |
...
| **Hard lossless terminal memory budgets** | Per-tab scrollback persists before shrinking, oversized derived caches regenerate on demand, and only fully invisible window renderers are evicted; PTY state and the last frame stay resident |
...
- Under memory pressure, the immediately previous tab remains warm as a bounded MRU exception so switching back is instant; older background tabs can still flush scrollback and shed render caches.
...
- Terminal splits share one focused-pane interaction policy across SwiftUI construction and imperative refreshes: only the focused leaf owns keyboard input and the window-level Metal renderer, while visible siblings retain their lifecycle and render through an immediately refreshed CPU fallback. Renderer movement is a generation-gated two-phase handoff: the incoming pane remains visible and interactive on its CPU-backed frame until the matching first GPU frame completes, and stale completions cannot reveal an older pane. Native AppKit first-responder changes are the authoritative focus signal, and delayed SwiftUI updates must revalidate renderer claims against the split controller's live focused-pane ID.
- Visible terminal siblings remain mouse-interactive even when they are not the keyboard owner: clicking one transfers AppKit focus before selection, and copy/paste actions route to that pane rather than the previously focused terminal. Copy-on-select also resets when a new selection begins, preventing a repeated selection from leaving stale clipboard contents.
...
- **Display-paced iPhone terminal rendering** — a serial actor ingests PTY bytes and extracts Rust grids off-main, while a one-shot 60/120 Hz display link publishes only the newest immutable frame. Layout callbacks cannot masquerade as user scrolling and pin playback to stale history, and frame observation stays inside the renderer subtree so live content does not invalidate the tab picker or input chrome; overloaded output still fast-forwards safely to a checkpoint. Sequence-correlated diagnostics time Mac capture/send, iOS receive/apply, engine mutation, state publication, view update, Core Graphics draw completion, and the next compositor refresh opportunity without per-frame disk logging.
- **Media-style terminal transport** — both PTY read accumulation and protocol sending use bounded 4 ms micro-batches, keeping normal Mac-side transport handoff within one 120 Hz display frame. Snapshots act as keyframes for initial state and recovery, authenticated timing metadata measures sender/network latency, and a side-effect-free checkpoint request resynchronizes overloaded clients.
...
- **iPhone frame-admission diagnostics** — rolling streaming summaries separate raw receipt from admitted frames, count decode and decrypt failures, and classify admitted handshake, inventory, snapshot, output, and grid traffic so transport activity cannot be mistaken for successfully delivered terminal content.
...
- **Peer-confirmed iPhone session recovery** — a fresh iPhone HELLO from the trusted identity replaces a helper crypto epoch that survived the app relaunch without rotating the Mac nonce or discarding cached terminal state. Both peers remain provisional until an encrypted SESSION_READY round trip confirms the same epoch; input, tab requests, approvals, and telemetry stay queued until then, preventing a false connected state from becoming an endless empty inventory.
...
- Pane-safe remote input — prompt answers validate both current prompt and pane identity; direct typing carries the streamed pane ID. Stale/closed targets and ambiguous legacy split input are rejected, all split panes publish state changes, and delayed protected-action approvals cannot move to a replacement terminal.
...
- Menu waiting-status gating — a cursor-marked menu row ("❯ 1.") flips the session to waiting-for-input so keyword-less menus get scanned, and a 1-second recheck while prompt cards are live keeps arrow-navigation responses tracking the on-screen cursor. Parsed positive/negative detections are cached by output revision, input time, provider, and terminal identity: unchanged panes need no new tail capture or parsing, while cursor redraws invalidate immediately. The bounded cache is excluded from UI observation and cleared on remote disconnect.
...
- Truthful and stable iPhone tab inventory — Chau7 Remote distinguishes encrypted-session readiness from tab-inventory readiness, shows syncing until the first live or cached inventory arrives, and suppresses duplicate or reorder-only observable updates. macOS also gates inventory frames by semantic descriptor equality and no longer attaches a full terminal keyframe to generic tab-model notifications, so shell output cannot rebuild an open picker; reconnect resets the gate before the first authoritative inventory.
...
- Selected-tab-only streaming — macOS streams terminal output and snapshots only for the tab explicitly selected on iPhone, including Mac-background sessions. The foreground phone's one subscribed terminal uses the event-driven PTY drain even while its Mac view is hidden, while hidden-view rendering and surrounding UI invalidation stay disabled. Phone selection survives inventories and reconnects without changing Mac focus; approvals-only/background mode releases the realtime drain, and non-subscribed tabs stay metadata/activity/approvals-only until switched to.
...
- **Guarded local rebuild/relaunch** — `Scripts/rebuild-and-relaunch.sh` validates a clean, current checkout and the embedded helper revision, builds before stopping the running app, quits with bounded graceful escalation, and installs through same-volume atomic replacement with a rollback backup. It supports dry-run, quit-only, debug/no-install, and explicit dirty/stale-source overrides while leaving session data untouched. Missing or mismatched bundle identity, app SHA, or Go `vcs.revision=<sha>` helper metadata fails verification before installation.
- **Bounded monitoring subprocesses** — Process/resource scanners and Aethyme delivery monitoring drain both output pipes concurrently with an aggregate size cap, deadline, and closed stdin. Partial/unsuccessful results cannot masquerade as valid scanner output; incomplete Aethyme responses are not decoded or acknowledged and pause remaining commands until the next cycle. Only the short-lived owned command is terminated on limits, never a terminal session or process group.
...
- **Independent hang captures** — a companion Chau7 command process observes a disk heartbeat and captures one `sample` plus JSON manifest for a hard main-thread stall even when the GUI cannot log or recover. It uses signal 0 only to test parent liveness and never terminates or relaunches the app.
...
| Main-thread hang samples | `~/Library/Application Support/Chau7/HangDiagnostics/` |
