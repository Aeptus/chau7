...
- **MAGI command surface** — `magi`/`MAGI` supports direct questions, `ask`, `doctor`, `config`, `replay <run-id>`, and `share <run-id>` with local artifacts only in v1, plus `--mode engineering|generic` for explicit verdict mode selection.
...
- **MAGI production artifacts** — running stages write lightweight decision JSON checkpoints, while failed, interrupted, vetoed, deadlocked, and completed terminal states carry checkpoint/failure metadata across decision markdown, decision JSON, transcript/replay JSONL, graph JSON, and local share HTML (a terminal replay is rendered on demand by the `replay` command).
