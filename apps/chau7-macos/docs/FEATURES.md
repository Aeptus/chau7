...
- Responsive settings shell opens on a Start Here dashboard for launch/profile/Agent Control/remote/notification/permission/path health, organizes panes into goal-oriented groups (General, Appearance, Terminal, AI Workflows, Automation, Safety & Privacy), and keeps the sidebar, search results, section copy, and form controls usable while resizing, with adaptive rows, wrapping text for longer localized labels and accessibility descriptions, a compact native titlebar profile selector that defaults to "Default Settings" and owns profile load/save actions, tighter shared golden-ratio SwiftUI style tokens, small-control row defaults, shared nested indentation, and compact reusable status summaries for app state and protocol support.
