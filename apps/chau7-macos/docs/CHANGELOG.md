...
### Fixed
- **Metal TUI Symbols Keep ANSI Color**: Metal color-glyph detection now honors emoji presentation before sampling embedded RGBA glyph data, so Codex/Claude text symbols like `✳` and `⏺` remain ANSI-tinted while real emoji such as `✅` and `🧪` still render with embedded color.
