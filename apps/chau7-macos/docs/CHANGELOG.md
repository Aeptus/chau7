...
- **Tabbar Height No Longer Doubles in Fullscreen**: `TabBarToolbarDelegate.applySizing` set the toolbar item's height to `max(OverlayLayout.tabBarHeight, window.frame.height - window.contentLayoutRect.height)`. In window mode the second term is the native titlebar height (~28pt) and the `max` is a no-op — the intent was to grow the bar so tabs sit aligned with the traffic-light buttons. In fullscreen there is no titlebar, but `contentLayoutRect` still excludes AppKit's safe-area / menu-bar reveal inset, which on a notch MacBook reaches ~60pt — the tabbar visibly doubled in height the moment the window entered fullscreen. The branch is now gated on `window.styleMask.contains(.fullScreen)`: fullscreen windows pin to `OverlayLayout.tabBarHeight`, window mode keeps the existing titlebar-aligned behavior. Latent since 2026-03-03 (`fd59cda` "Add tab bar disappearance diagnostics on resize") — never tripped before because window mode hides the bug behind a benign `max`.
...
- **Multi-Window Startup Live-Frame Race**: Per-window `OverlayTabsModel` registers its `terminalSessionVisibleFrameReady` observer in `init`, but the SwiftUI terminal view's first paint can fire the notification synchronously *before* the second window's init completes. The observer never sees it, the `StartupRestoreCoordinator` stops waiting, and the 5 s fallback synthesizes a live-frame call for every multi-window launch (`fallback_5s_no_live_frame` warning, both windows, exactly 5 s elapsed). The model now exposes `replaySelectedTabLiveFrameIfAlreadyPresented(reason:)`, called from `AppDelegate.showOverlayWindow` immediately after `noteWindowVisible` arms the fallback timer; if the selected session has already presented (`firstFramePresentedAt != nil`), it dispatches the live-frame note synchronously, and the fallback timer cancels itself. Idempotent (the coordinator dedups), safe on single-window launches (the observer fires naturally there).
