...
- **Runbook Session Plan Binding**: The PM dashboard now prefers `.chau7/sessions/<session-id>/plan.md` when exactly one active session owns a repo, keeps autosave scoped to managed Chau7 companion plans only, and tracks duplicate markdown code blocks by block position so reruns do not clobber each other’s status.
...
- **Turn-Scoped Runtime Event Journal**: Runtime journals now record explicit `user_input`, correlated `tool_use`/`tool_result`, turn-scoped `output_chunk`, and per-turn query support so orchestrators can reconstruct a single turn’s causality without scraping mixed session history.
...
- **Runtime Turn Boundary Integrity**: `completeTurn()` now rejects completion without an active turn instead of silently journaling a dangling completion, and `turn_completed` payloads now include `turn_duration_ms` so consumers can reason about turn timing without external clocks.
- **Runtime Tool Result Fidelity**: Runtime event journals now carry correlation IDs across tool use, approvals, and tool results; tool results also include success, exit code, error text, output preview, file path, and duration metadata. Output chunks are emitted during the turn with `turn_id` and `chunk_index`, and session journal/output limits are configurable through runtime settings.
