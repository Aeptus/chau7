...
### Added
- **Workspace-Wide Startup Restore Gate**: Chau7 now keeps the splash/logo visible while it restores and renders every saved tab once behind the scenes, and only reveals windows after the startup restore queue and restore-bootstrap work have drained. This replaces the old “show selected windows first, finish background restore later” startup contract.
