...
- **MCP `tab_wait_ready`**: New MCP wait helper blocks until a tab is prompt-ready for immediate execution and returns the last observed tab status snapshot on timeout, so eval orchestrators can diagnose slow shell startup without maintaining their own polling heuristic.

### Fixed
- **MCP Tab Readiness Contract**: `tab_status` and `tab_create` now expose explicit execution-readiness fields (`ready_for_exec`, `readiness_reason`, `has_terminal_view`), and `tab_exec` is documented as bootstrap-queueable so launchers can start commands immediately instead of serially waiting on shell-loading heuristics.
