...
public enum MagiConfigTOMLCodec {
    public static func encode(_ config: MagiConfig) -> String {
        let lines: [String] = [
            "# MAGI Configuration",
            "# Generated by Chau7. Global runtime policy only.",
            "# Council members live in councils/<council-id>.toml.",
            "",
...
public enum MagiCouncilArtFile {
    ...
    public static func configFileName(for councilID: String) -> String {
        "\(sanitize(councilID)).toml"
    }
...
public enum MagiFirstRunInstaller {
    ...
    public static func install(
        ...
        let councilConfig = MagiCouncilConfiguration.defaultMagi(
            councilID: config.defaultCouncilID,
            displayName: councilArt.displayName,
            members: config.members,
            defaultReasoning: config.defaultReasoning
        )
        try write(
            MagiCouncilConfigTOMLCodec.encode(councilConfig),
            to: paths.councilConfigPath(for: config.defaultCouncilID),
            fileManager: fileManager,
            overwrite: overwrite,
            createdPaths: &createdPaths,
            skippedPaths: &skippedPaths
