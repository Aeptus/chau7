...
final class TerminalSessionModel {
    ...
    func attachRustTerminal(_ view: RustTerminalView) {
        ...
        // Wire up foreign OSC 9 desktop notifications (ESC]9;<message>BEL that
        // does NOT start with chau7;). The Codex CLI TUI emits these for every
        // approval/input/plan-mode/agent-turn-complete event. Classify by
        // message prefix and route through the standard AIEvent pipeline so
        // notifications fire on the correct tab.
        view.onForeignDesktopNotification = { [weak self] message in
            guard let self else { return }
            handleForeignDesktopNotification(message)
        }
    ...
    static func preInitialize() {
        ShellLaunchConfigurator.prepareShellIntegration()
    ...
    static func getShellIntegrationDir() -> String? {
        return ShellLaunchConfigurator.shellIntegrationDirectory
    }
    ...
    static func resolveStartDirectory(_ rawValue: String) -> String {
        ShellLaunchConfigurator.resolveStartDirectory(rawValue)
    }
    ...
    private func startDirectoryForLaunch() -> String {
        ShellLaunchConfigurator.startDirectoryForLaunch(
            requested: currentDirectory,
            defaultDirectory: Self.defaultStartDirectory()
        )
    }

    func defaultShell() -> String {
        let settings = FeatureSettings.shared
        return ShellLaunchConfigurator.shellPath(
            for: settings.shellType,
    ...
    func buildEnvironment() -> [String] {
        let settings = FeatureSettings.shared
        let ctoEnabled = settings.tokenOptimizationMode != .off

        return ShellLaunchConfigurator.launchEnvironment(ShellLaunchConfigurator.LaunchEnvironmentInputs(
            pathValue: launchPATHValue(),
            shellPath: defaultShell(),
            startDirectory: startDirectoryForLaunch(),
            startupCommand: settings.startupCommand,
            isLsColorsEnabled: settings.isLsColorsEnabled,
            integrationDir: Self.getShellIntegrationDir(),
            proxyCorrelationSessionID: proxyCorrelationSessionID,
            tabID: ownerTabID?.uuidString ?? tabIdentifier,
            projectDirectory: currentDirectory,
            aiEventsLogPath: sharedEventsLogPathForEnvironment(),
            cto: ctoEnabled
                ? ShellLaunchConfigurator.CTOLaunchContext(
                    sessionID: tabIdentifier,
                    commandLogPath: CTOManager.shared.commandLogPath.path
                )
                : nil,
            apiAnalytics: settings.isAPIAnalyticsEnabled
                ? ShellLaunchConfigurator.APIAnalyticsProxyContext(
                    port: settings.apiAnalyticsPort,
                    includeOpenAI: settings.apiAnalyticsIncludeOpenAI
                )
                : nil
        ))
    }
    ...
    func shellArguments() -> [String] {
        ShellLaunchConfigurator.shellArguments(
            shellPath: defaultShell(),
            integrationDir: Self.getShellIntegrationDir()
        )
    }
    ...
    struct ForeignNotificationClassification: Equatable {
        let source: AIEventSource
        let type: String
        let tool: String
    }
