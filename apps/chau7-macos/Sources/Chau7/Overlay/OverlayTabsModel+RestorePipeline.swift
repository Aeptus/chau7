...
extension OverlayTabsModel {
    ...
    static func restoreSavedTabs(appModel: AppModel) -> RestorableTabsPayload? {
        ...
        if let multiState = Persist.decodeLogged(
            ...
            !primaryWindowStates.isEmpty {
            let mergedWindows = TabStateBackupStore.mergedWindowStatesWithBackupFallbacks(baseWindows: multiState.windows)
            TabStateBackupStore.maybeRepairUserDefaultsMultiWindowState(
        ...
        if let singleWindowStates = Persist.decodeLogged(
            ...
            let mergedWindows = TabStateBackupStore.mergedWindowStatesWithBackupFallbacks(baseWindows: [singleWindowStates])
            TabStateBackupStore.maybeRepairUserDefaultsSingleWindowState(
    ...
    static func restoreSavedTabsFromBackups(appModel: AppModel) -> RestorableTabsPayload? {
        guard let windows = TabStateBackupStore.mergedBackupWindowStatesFromCandidates() else { return nil }
    ...
    // MARK: - TabStateBackupStore forwarders

    //
    // The backup persistence/restore/merge/repair machinery moved to
    // `TabStateBackupStore` (pure statics, no model instance state). These
    // thin forwarders keep the historical `OverlayTabsModel.<fn>` entry
    // points stable for external callers (`AppDelegate`, the test suite)
    // that predate the extraction — the implementations now live in one
    // place. New call sites should prefer `TabStateBackupStore` directly.
    ...
    static func persistWindowStateBackups(windowStates: [[SavedTabState]], reason: TabStateSaveReason) {
        TabStateBackupStore.persistWindowStateBackups(windowStates: windowStates, reason: reason)
    }

    static func clearPersistedWindowState() {
        TabStateBackupStore.clearPersistedWindowState()
    }

    static func tabStateBackupRootURL() -> URL? {
        TabStateBackupStore.tabStateBackupRootURL()
    }
