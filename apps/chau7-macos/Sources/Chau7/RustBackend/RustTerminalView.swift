...
final class RustTerminalFFI {
    ...
    /// Bound function pointers for libchau7_terminal.dylib. All dlopen /
    /// ABI-probe / dlsym plumbing lives in `RustTerminalSymbolTable`.
    private static var functions: RustTerminalSymbolTable?
    ...
    private static func ensureLoaded() {
        ...
        let candidates = RustTerminalSymbolTable.libraryCandidates()
        ...
        for (index, path) in candidates.enumerated() {
            ...
            guard let loaded = RustTerminalSymbolTable.load(path: path) else { continue }
            dylibHandle = loaded.handle
    ...
    /// Get the full terminal buffer text (visible + scrollback) with ANSI styling.
    func fullBufferAnsiText() -> String? {
        guard let getFullBufferAnsiTextFn = Self.functions?.getFullBufferAnsiText,
              let freeStringFn = Self.functions?.freeString else {
            Log.trace("RustTerminalFFI[\(instanceId)]: fullBufferAnsiText - Function not available")
            return nil
        }

        guard let ptr = getFullBufferAnsiTextFn(terminal) else {
            Log.trace("RustTerminalFFI[\(instanceId)]: fullBufferAnsiText - No text returned")
            return nil
        }
        defer { freeStringFn(ptr) }
...
final class RustTerminalView: NSView {
    ...
    static func renderGeometry(bounds: CGRect, cellSize: CGSize) -> TerminalRenderGeometry {
        TerminalRenderGeometry.resolve(
            bounds: bounds,
            inset: terminalInset,
            cellSize: cellSize
        )
    ...
    override func layout() {
        ...
        if geometry.isClamped {
            Log.warn(
                "RustTerminalView[\(viewId)]: layout - clamped absurd resize target rawCols=\(geometry.rawCols) rawRows=\(geometry.rawRows) bounds=\(bounds) contentBounds=\(geometry.contentBounds) cellWidth=\(cellWidth) cellHeight=\(cellHeight)"
            )
        }
        ...
        if geometry.canResizePTY {
            let newCols = geometry.cols
            let newRows = geometry.rows
            if newCols != cols || newRows != rows {
                Log
                    .info(
                        "RustTerminalView[\(viewId)]: layout - Resizing from \(cols)x\(rows) to \(newCols)x\(newRows) verticalRemainder=\(String(format: "%.1f", geometry.verticalRemainder))px horizontalRemainder=\(String(format: "%.1f", geometry.horizontalRemainder))px cellHeight=\(cellHeight)"
                    )
                cols = newCols
                rows = newRows
    ...
    func desiredPollingMode() -> VisibleTerminalPollingMode {
        VisibleTerminalPollingPolicy.mode(
            for: VisibleTerminalPollingContext(
                ...
                isWindowMiniaturized: window?.isMiniaturized ?? false
            )
        )
    }
    ...
    func pointToCell(_ point: NSPoint) -> (col: Int32, row: Int32) {
        let geometry = currentRenderGeometry
        let cell = geometry.clampedCell(for: point)
        let col = Int32(cell.col)
        let row = Int32(cell.row)
        return (col, row)
    }
