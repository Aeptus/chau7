...
struct SettingsPageSummaryView: View {
    ...
    private var summaryItems: [SettingsStatusItem] {
        ...
        case .display:
            return [
                statusItem("syntax", L("settings.appearance.syntaxHighlighting", "Syntax Highlighting"), enabledDisabled(settings.isSyntaxHighlightEnabled), icon: "curlybraces", tone: enabledTone(settings.isSyntaxHighlightEnabled)),
                statusItem("urls", L("settings.appearance.clickableURLs", "Clickable URLs"), enabledDisabled(settings.isClickableURLsEnabled), icon: "link", tone: enabledTone(settings.isClickableURLsEnabled)),
                statusItem("images", L("settings.appearance.inlineImages", "Inline Images"), enabledDisabled(settings.isInlineImagesEnabled), icon: "photo", tone: enabledTone(settings.isInlineImagesEnabled)),
                statusItem("json", L("settings.appearance.prettyPrintJSON", "Pretty Print JSON"), enabledDisabled(settings.isJSONPrettyPrintEnabled), icon: "curlybraces.square", tone: enabledTone(settings.isJSONPrettyPrintEnabled))
            ]
        case .windows:
            return [
        ...
        case .repositories:
            return [
                statusItem("grouping", L("settings.tabs.repoGrouping", "Repo Grouping"), settings.repoGroupingMode.displayName, icon: "rectangle.3.group", tone: settings.repoGroupingMode == .off ? .disabled : .enabled),
                statusItem("search", L("settings.repositories.semanticSearch", "Semantic Search"), enabledDisabled(settings.isSemanticSearchEnabled), icon: "magnifyingglass", tone: enabledTone(settings.isSemanticSearchEnabled)),
                countItem("recent", L("settings.repositories.recent", "Recent Repos"), settings.recentRepoRoots.count, icon: "clock.arrow.circlepath"),
                statusItem("defaultDirectory", L("settings.general.defaultDirectory", "Default Directory"), compactPath(settings.defaultStartDirectory.isEmpty ? "~" : settings.defaultStartDirectory), icon: "folder", tone: .neutral)
            ]
        case .shell:
            return [
                statusItem("shell", L("settings.terminal.shell", "Shell"), shellDisplayName, icon: "terminal", tone: .neutral),
                statusItem("startup", L("settings.terminal.startupCommand", "Startup Command"), configuredUnconfigured(!settings.startupCommand.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty), icon: "play", tone: configuredTone(!settings.startupCommand.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty)),
                ...
                statusItem("bell", L("settings.terminal.bell", "Bell"), enabledDisabled(settings.bellEnabled), detail: settings.bellEnabled ? settings.bellSound : nil, icon: "bell", tone: enabledTone(settings.bellEnabled))
            ]
        case .scrollbackPerf:
            return [
                ...
                statusItem("renderer", L("settings.performance.renderer", "Renderer"), settings.useMetalRenderer ? "Metal" : "CoreGraphics", icon: "speedometer", tone: settings.useMetalRenderer ? .enabled : .neutral),
        ...
        case .dangerousCommands:
            return [
                statusItem("protectChau7", L("settings.dangerous.protectChau7", "Protect Chau7"), enabledDisabled(settings.dangerousCommandProtectChau7Enabled), detail: protectionLevelLabel, icon: "shield", tone: enabledTone(settings.dangerousCommandProtectChau7Enabled)),
                ...
                statusItem("lowPower", L("settings.dangerous.lowPower", "Low Power"), enabledDisabled(settings.dangerousOutputHighlightLowPowerEnabled), icon: "battery.25", tone: enabledTone(settings.dangerousOutputHighlightLowPowerEnabled))
            ]
        case .graphics:
            return [
                ...
                statusItem("kitty", L("settings.graphics.kitty", "Kitty Graphics"), enabledDisabled(graphics.isKittyGraphicsEnabled), icon: "photo.stack", tone: enabledTone(graphics.isKittyGraphicsEnabled)),
                statusItem("inlineImages", L("settings.appearance.inlineImages", "Inline Images"), enabledDisabled(settings.isInlineImagesEnabled), icon: "rectangle.on.rectangle", tone: enabledTone(settings.isInlineImagesEnabled)),
                statusItem("cache", L("settings.graphics.cache", "Cache"), "\(graphics.kittyCacheLimitMB) MB", icon: "externaldrive", tone: .neutral)
            ]
        case .keyboardMouse:
            return [
                statusItem("shortcutHelper", L("settings.shortcuts.helper", "Shortcut Helper"), enabledDisabled(settings.isShortcutHelperHintEnabled), icon: "keyboard", tone: enabledTone(settings.isShortcutHelperHintEnabled)),
                statusItem("copyOnSelect", L("settings.input.copyOnSelect", "Copy on Select"), enabledDisabled(settings.isCopyOnSelectEnabled), icon: "doc.on.doc", tone: enabledTone(settings.isCopyOnSelectEnabled)),
                statusItem("cmdClick", L("settings.input.cmdClickPaths", "Cmd-Click Paths"), enabledDisabled(settings.isCmdClickPathsEnabled), icon: "cursorarrow.click", tone: enabledTone(settings.isCmdClickPathsEnabled)),
                statusItem("mouseReporting", L("settings.input.mouseReporting", "Mouse Reporting"), enabledDisabled(settings.isMouseReportingEnabled), icon: "computermouse", tone: enabledTone(settings.isMouseReportingEnabled))
            ]
        case .snippetsTools:
            return [
                statusItem("snippets", L("settings.snippetsTools.snippets", "Snippets"), enabledDisabled(settings.isSnippetsEnabled), icon: "text.badge.plus", tone: enabledTone(settings.isSnippetsEnabled)),
                statusItem("repoSnippets", L("settings.snippetsTools.repoSnippets", "Repo Snippets"), enabledDisabled(settings.isRepoSnippetsEnabled), icon: "folder.badge.plus", tone: enabledTone(settings.isRepoSnippetsEnabled)),
                statusItem("clipboard", L("settings.snippetsTools.clipboardHistory", "Clipboard History"), enabledDisabled(settings.isClipboardHistoryEnabled), detail: settings.isClipboardHistoryEnabled ? String(format: L("settings.summary.items", "%d items"), settings.clipboardHistoryMaxItems) : nil, icon: "clipboard", tone: enabledTone(settings.isClipboardHistoryEnabled)),
                statusItem("bookmarks", L("settings.snippetsTools.bookmarks", "Bookmarks"), enabledDisabled(settings.isBookmarksEnabled), icon: "bookmark", tone: enabledTone(settings.isBookmarksEnabled))
            ]
        case .editor:
            return [
                statusItem("editor", L("settings.editor.defaultEditor", "Default Editor"), settings.defaultEditor, icon: "doc.text", tone: .neutral),
                statusItem("internal", L("settings.editor.internalEditor", "Internal Editor"), enabledDisabled(settings.cmdClickOpensInternalEditor), icon: "square.split.2x1", tone: enabledTone(settings.cmdClickOpensInternalEditor)),
                statusItem("splitPanes", L("settings.windows.splitPanes", "Split Panes"), enabledDisabled(settings.isSplitPanesEnabled), icon: "rectangle.split.2x1", tone: enabledTone(settings.isSplitPanesEnabled)),
                statusItem("semanticSearch", L("settings.repositories.semanticSearch", "Semantic Search"), enabledDisabled(settings.isSemanticSearchEnabled), icon: "magnifyingglass", tone: enabledTone(settings.isSemanticSearchEnabled))
            ]
        case .minimalMode:
            return [
                ...
                statusItem("tabBar", L("settings.minimal.hideTabBar", "Hide Tab Bar"), visibleHidden(minimalMode.hideTabBar), icon: "rectangle.topthird.inset.filled", tone: enabledTone(minimalMode.hideTabBar)),
                ...
                statusItem("statusBar", L("settings.minimal.hideStatusBar", "Hide Status Bar"), visibleHidden(minimalMode.hideStatusBar), icon: "rectangle.bottomthird.inset.filled", tone: enabledTone(minimalMode.hideStatusBar))
            ]
        case .aiDetection:
            return [
        ...
        case .tokenOptimization:
            return [
                statusItem("mode", L("cto.settings.mode.label", "Mode"), settings.tokenOptimizationMode.displayName, icon: "bolt.horizontal.circle", tone: settings.tokenOptimizationMode == .off ? .disabled : .enabled),
                statusItem("prefix", L("settings.ai.cto.enabled", "Enable Input Prefix"), enabledDisabled(settings.isCTOEnabled), detail: settings.isCTOEnabled ? settings.ctoPrefix : nil, icon: "wand.and.stars", tone: enabledTone(settings.isCTOEnabled)),
                countItem("overrides", L("cto.settings.perTab", "Per-Tab Control"), settings.ctoTabOverrides.count, icon: "rectangle.stack"),
                statusItem("indicator", L("settings.tabs.allowCTOToggle", "Allow Context Optimization Toggle in Hover Card"), enabledDisabled(settings.allowTabCTOToggle), icon: "switch.2", tone: enabledTone(settings.allowTabCTOToggle))
            ]
        case .mcpControl:
            return [
                ...
                statusItem("approval", L("settings.mcp.approval", "Require Approval"), enabledDisabled(settings.mcpRequiresApproval), icon: "lock.shield", tone: enabledTone(settings.mcpRequiresApproval)),
        ...
        case .remoteControl:
            return [
                statusItem("enabled", L("settings.remote.enable", "Enable Remote Access"), enabledDisabled(settings.isRemoteEnabled), icon: "antenna.radiowaves.left.and.right", tone: enabledTone(settings.isRemoteEnabled)),
                statusItem("agent", L("settings.remote.agent", "Remote Agent"), remote.isAgentRunning ? L("status.running", "Running") : L("status.stopped", "Stopped"), detail: remote.lastError, icon: "dot.radiowaves.left.and.right", tone: remoteStatusTone),
                statusItem("relay", L("settings.remote.relay", "Relay"), compactPath(settings.remoteRelayURL), icon: "network", tone: settings.remoteRelayURL.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? .warning : .neutral),
        ...
        case .apiProxy:
            return [
                statusItem("enabled", L("settings.proxy.enable", "Enable API Analytics"), enabledDisabled(settings.isAPIAnalyticsEnabled), icon: "chart.bar.xaxis", tone: enabledTone(settings.isAPIAnalyticsEnabled)),
                ...
                statusItem("privacy", L("settings.proxy.logPrompts", "Log Prompt Previews"), enabledDisabled(settings.apiAnalyticsLogPrompts), icon: "lock.open", tone: settings.apiAnalyticsLogPrompts ? .warning : .enabled)
            ]
        case .promptInjection:
            return [
                statusItem("global", L("settings.injection.global", "All Repositories"), enabledDisabled(injectionStore.globalRule != nil), icon: "globe", tone: enabledTone(injectionStore.globalRule != nil)),
                ...
                statusItem("proxy", L("settings.apiProxy", "API Tracking"), enabledDisabled(settings.isAPIAnalyticsEnabled), detail: L("settings.summary.proxyNeeded", "Proxy applies AI context rules."), icon: "network", tone: enabledTone(settings.isAPIAnalyticsEnabled))
            ]
        case .notifications:
            return [
                statusItem("permission", L("settings.productivity.permissions.notifications", "Notifications"), model.notificationStatus, detail: model.notificationWarning, icon: "bell.badge", tone: model.notificationWarning == nil ? .enabled : .warning),
        ...
        case .history:
            return [
                statusItem("persistent", L("settings.history.enable", "Enable Persistent History"), enabledDisabled(persistentHistoryEnabled), icon: "clock.arrow.circlepath", tone: enabledTone(persistentHistoryEnabled)),
                countItem("maxRecords", L("settings.history.maxRecords", "Maximum Records"), historyMaxRecords, icon: "externaldrive"),
                statusItem("telemetry", L("settings.logs.telemetry", "AI Telemetry & Transcripts"), telemetryRetentionText, icon: "chart.bar.doc.horizontal", tone: settings.telemetryRetentionDays == 0 ? .warning : .neutral),
        ...
        case .logsHistory:
            return [
                statusItem("historyLogs", L("settings.logs.monitorHistoryLogs", "Monitor History Logs"), enabledDisabled(model.isIdleMonitoring), icon: "clock.arrow.circlepath", tone: enabledTone(model.isIdleMonitoring)),
                statusItem("terminalLogs", L("settings.logs.monitorTerminalLogs", "Monitor Terminal Logs"), enabledDisabled(model.isTerminalMonitoring), icon: "terminal", tone: enabledTone(model.isTerminalMonitoring)),
    ...
    private var historyMaxRecords: Int {
        ...
        return max(10_000, min(stored > 0 ? stored : 50_000, 100_000))
    }
