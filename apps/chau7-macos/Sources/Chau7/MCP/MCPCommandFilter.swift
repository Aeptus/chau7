...
///   3. Everything else — route to user for approval (Mac alert + iOS)
enum MCPCommandFilter {
    ...
    static func check(_ command: String) -> MCPCommandVerdict {
        ...
        // Permissive mode — everything allowed
        if mode == .allowAll { return .allowed }
        ...
        for base in baseCommands {
            ...
            // Both allowlist and askUnlisted: only explicitly allowed commands pass
    ...
    private static func firstToken(_ cmd: String) -> String? {
        ...
        let passthrough: Set<String> = ["sudo", "command", "env", "nohup", "nice", "time", "exec"]
