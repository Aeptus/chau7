...
@MainActor protocol NotificationActionDelegate: AnyObject {
    func focusTab(forTool tool: String)
    func styleTab(forTool tool: String, preset: String, config: [String: String])
    func badgeTab(forTool tool: String, text: String, color: String)
    func insertSnippet(id: String, forTool tool: String, autoExecute: Bool)
...
final class NotificationActionExecutor {
    ...
    private func executeFocusWindow(_ ctx: ActionContext) {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            if focusTab {
                self?.delegate?.focusTab(forTool: ctx.event.tool)
            }
        }
    }
    ...
    private func executeBadgeTab(_ ctx: ActionContext) {
        ...
        DispatchQueue.main.async { [weak self] in
            self?.delegate?.badgeTab(forTool: ctx.event.tool, text: badgeText, color: badgeColor)
        }
    }
    ...
    private func executeStyleTab(_ ctx: ActionContext) {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            self.delegate?.styleTab(forTool: tool, preset: stylePreset, config: config)

            if autoClearSeconds > 0 {
                let workItem = DispatchWorkItem { [weak self] in
                    self?.pendingStyleClears.removeValue(forKey: tool)
                    self?.delegate?.styleTab(forTool: tool, preset: "clear", config: [:])
                }
    ...
    private func executeExecuteSnippet(_ ctx: ActionContext) {
        ...
        DispatchQueue.main.async { [weak self] in
            self?.delegate?.insertSnippet(id: snippetId, forTool: ctx.event.tool, autoExecute: autoExecute)
        }
    }
...
final class NotificationActionAdapter: NotificationActionDelegate {
    ...
    func focusTab(forTool tool: String) {
        overlayModel?.focusTabByTool(tool)
    }

    func styleTab(forTool tool: String, preset: String, config: [String: String]) {
        overlayModel?.applyNotificationStyle(forTool: tool, stylePreset: preset, config: config)
    }

    func badgeTab(forTool tool: String, text: String, color: String) {
        overlayModel?.setBadgeByTool(tool, text: text, color: color)
    }

    func insertSnippet(id: String, forTool tool: String, autoExecute: Bool) {
        overlayModel?.insertSnippetById(id, forTool: tool, autoExecute: autoExecute)
    }
