...
final class TabHoverCardAnalyticsModelTests: XCTestCase {
    ...
    /// 5s matches the prevailing default across the suite. A success-path wait
    /// ends as soon as its condition holds, so the ceiling only costs time when
    /// the assertion was going to fail anyway; at 1s this polled too few times
    /// to survive CPU contention.
