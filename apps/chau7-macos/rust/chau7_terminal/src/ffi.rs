...
pub unsafe extern "C" fn chau7_terminal_create(
    ...
    unsafe {
        ...
        info!(
            "chau7_terminal_create(cols={}, rows={}, shell={:?})",
            cols, rows, shell
        );
        ...
        let shell_str = if shell.is_null() {
            ...
            match CStr::from_ptr(shell).to_str() {
                ...
                Err(e) => {
                    error!(
                        "chau7_terminal_create: Invalid shell string (not UTF-8): {}",
                        e
                    );
...
pub unsafe extern "C" fn chau7_terminal_create_with_env(
    ...
    unsafe {
        ...
        info!(
            "chau7_terminal_create_with_env(cols={}, rows={}, env_count={})",
            cols, rows, env_count
        );
        ...
        let shell_str = if shell.is_null() {
            ...
            match CStr::from_ptr(shell).to_str() {
                ...
                Err(e) => {
                    error!(
                        "chau7_terminal_create_with_env: Invalid shell string: {}",
                        e
                    );
                    return std::ptr::null_mut();
                }
        ...
        if env_count > 0 && !env_keys.is_null() && !env_values.is_null() {
            for i in 0..env_count {
                ...
                if key_ptr.is_null() || value_ptr.is_null() {
                    warn!(
                        "chau7_terminal_create_with_env: Null env pointer at index {}",
                        i
                    );
                    continue;
                }
                ...
                debug!(
                    "chau7_terminal_create_with_env: env[{}] = {}={}",
                    i, key, value
                );
        ...
        match Chau7Terminal::new_with_env(cols, rows, shell_str, &env_refs) {
            Ok(terminal) => {
                let ptr = Box::into_raw(Box::new(terminal));
                info!(
                    "chau7_terminal_create_with_env: Success, returning {:p}",
                    ptr
                );
                ptr
            }
...
pub unsafe extern "C" fn chau7_terminal_free_grid(grid: *mut GridSnapshot) {
    unsafe {
        ...
        if !snapshot.cells.is_null() {
            ...
            trace!(
                "chau7_terminal_free_grid: returning cells to pool (len={}, cap={})",
...
pub unsafe extern "C" fn chau7_terminal_selection_start(
    ...
    unsafe {
        debug!(
            "chau7_terminal_selection_start({:p}, col={}, row={}, type={})",
            term, col, row, selection_type
        );
        if term.is_null() {
...
pub unsafe extern "C" fn chau7_terminal_selection_update(
    ...
    unsafe {
        trace!(
            "chau7_terminal_selection_update({:p}, col={}, row={})",
            term, col, row
        );
        if term.is_null() {
...
pub unsafe extern "C" fn chau7_terminal_cursor_position(
    ...
    unsafe {
        trace!(
            "chau7_terminal_cursor_position({:p}, {:p}, {:p})",
            term, col, row
        );
        if term.is_null() {
...
pub unsafe extern "C" fn chau7_terminal_get_last_output(
    ...
    unsafe {
        ...
        *out_len = len;
        trace!(
            "chau7_terminal_get_last_output: returning {} bytes at {:p}",
            len, ptr
        );
        ptr
    }
...
pub unsafe extern "C" fn chau7_terminal_inject_output(
    ...
    unsafe {
        trace!(
            "chau7_terminal_inject_output({:p}, {:p}, {})",
            term, data, len
        );
        if term.is_null() {
...
/// - `term` must be a valid pointer
#[unsafe(no_mangle)]
pub unsafe extern "C" fn chau7_terminal_is_application_cursor_mode(
    term: *mut Chau7Terminal,
) -> bool {
    unsafe {
...
/// - `term` must be a valid pointer
#[unsafe(no_mangle)]
pub unsafe extern "C" fn chau7_terminal_is_mouse_reporting_active(
    term: *mut Chau7Terminal,
) -> bool {
    unsafe {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn chau7_terminal_get_debug_state(
    term: *mut Chau7Terminal,
) -> *mut DebugState {
    unsafe {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn chau7_terminal_get_full_buffer_text(
    term: *mut Chau7Terminal,
) -> *mut c_char {
    unsafe {
...
/// - The returned pointer must be freed with `chau7_terminal_free_string`
#[unsafe(no_mangle)]
pub unsafe extern "C" fn chau7_terminal_get_tail_buffer_ansi_text(
    term: *mut Chau7Terminal,
    max_lines: usize,
    max_bytes: usize,
) -> *mut c_char {
    unsafe {
        trace!(
            "chau7_terminal_get_tail_buffer_ansi_text({:p}, max_lines={}, max_bytes={})",
            term, max_lines, max_bytes
        );
        if term.is_null() {
            warn!("chau7_terminal_get_tail_buffer_ansi_text: term is null");
            return std::ptr::null_mut();
        }
        let terminal = &*term;
        let text = terminal.tail_buffer_ansi_text(max_lines, max_bytes);
        match CString::new(text) {
            Ok(cstr) => cstr.into_raw(),
            Err(e) => {
                error!(
                    "chau7_terminal_get_tail_buffer_ansi_text: CString::new failed: {}",
                    e
                );
                std::ptr::null_mut()
            }
...
pub unsafe extern "C" fn chau7_terminal_get_pending_title(term: *mut Chau7Terminal) -> *mut c_char {
    unsafe {
        ...
        match title {
            Some(title) => {
                debug!(
                    "chau7_terminal_get_pending_title: returning title {:?}",
                    title
                );
                match CString::new(title) {
                    ...
                    Err(e) => {
                        warn!(
                            "chau7_terminal_get_pending_title: title contains null byte at pos {}",
                            e.nul_position()
                        );
                        std::ptr::null_mut()
                    }
...
pub unsafe extern "C" fn chau7_terminal_get_pending_exit_code(term: *mut Chau7Terminal) -> i32 {
    unsafe {
        ...
        match pending.take() {
            Some(code) => {
                info!(
                    "chau7_terminal_get_pending_exit_code: returning exit code {}",
                    code
                );
                code
            }
...
/// - `term` must be a valid pointer
#[unsafe(no_mangle)]
pub unsafe extern "C" fn chau7_terminal_is_echo_disabled(term: *mut Chau7Terminal) -> bool {
    unsafe {
...
/// - The returned string must be freed with `chau7_terminal_free_string`
#[unsafe(no_mangle)]
pub unsafe extern "C" fn chau7_terminal_get_link_url(
    term: *mut Chau7Terminal,
    link_id: u16,
) -> *mut c_char {
    unsafe {
        ...
        match terminal.get_link_url(link_id) {
            Some(url) => {
                trace!(
                    "chau7_terminal_get_link_url: link_id={} -> {:?}",
                    link_id, url
                );
                match CString::new(url) {
                    ...
                    Err(e) => {
                        warn!(
                            "chau7_terminal_get_link_url: URL contains null byte at pos {}",
                            e.nul_position()
                        );
                        std::ptr::null_mut()
                    }
...
/// - The returned string must be freed with `chau7_terminal_free_string`
#[unsafe(no_mangle)]
pub unsafe extern "C" fn chau7_terminal_get_pending_clipboard(
    term: *mut Chau7Terminal,
) -> *mut c_char {
    unsafe {
        ...
        match terminal.take_pending_clipboard_store() {
            Some(text) => {
                debug!(
                    "chau7_terminal_get_pending_clipboard: returning {} chars",
                    text.len()
                );
                match CString::new(text) {
                    Ok(cstr) => cstr.into_raw(),
                    Err(e) => {
                        warn!(
                            "chau7_terminal_get_pending_clipboard: text contains null byte at pos {}",
                            e.nul_position()
                        );
                        std::ptr::null_mut()
                    }
...
/// - `term` must be a valid pointer
#[unsafe(no_mangle)]
pub unsafe extern "C" fn chau7_terminal_has_clipboard_request(term: *mut Chau7Terminal) -> bool {
    unsafe {
...
/// - `text` must be a valid null-terminated C string
#[unsafe(no_mangle)]
pub unsafe extern "C" fn chau7_terminal_respond_clipboard(
    term: *mut Chau7Terminal,
    text: *const c_char,
) {
    unsafe {
        ...
        debug!(
            "chau7_terminal_respond_clipboard: responding with {} chars",
            clipboard_text.len()
        );
...
pub unsafe extern "C" fn chau7_terminal_get_pending_images(
    ...
    unsafe {
        ...
        debug!(
            "chau7_terminal_get_pending_images: {} images pending",
            images.len()
        );
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn chau7_terminal_free_images(array: *mut FFIImageArray) {
    unsafe {
...
pub unsafe extern "C" fn chau7_terminal_set_image_protocols(
    ...
    unsafe {
        trace!(
            "chau7_terminal_set_image_protocols({:p}, sixel={}, kitty={}, iterm2={})",
...
/// - `term` must be a valid pointer
#[unsafe(no_mangle)]
pub unsafe extern "C" fn chau7_terminal_has_pending_images(term: *mut Chau7Terminal) -> bool {
    unsafe {
