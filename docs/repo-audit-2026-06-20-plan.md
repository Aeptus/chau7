...
## Phase 5 — Cross-cutting consolidations (build the shared helper once)  ◑ IN PROGRESS (15/~22, 2026-06-20)
Done: 84, 93, 54, 62, 36, 10, 72, 66+68, 60, 58, 41, 40, 31 (TelemetryRun.applyContent), 55 (ProxyManager requestJSON/postJSON generics).
Remaining 7 — all risk-sensitive or high-touch (need a focused pass, esp. now that the loaded macOS test gate is ~16 min): 51 (Unix socket C-interop ×4 servers), 92 (OSC7 hot PTY path), 88 (DirtyRowTracker, entangled w/ #89), 25 (ExecutionReport, 25+ sites ×5 files), 11 (~1000-line overlay cut/paste), 47 (bug-report flow consolidation — user-facing), 32 (provider classification — call-site subsets differ).
